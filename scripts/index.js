...
import { Deck, Card, shuffleCards } from "./deck.js";
...
function init() {
    ...
    /**
     * Initialize and swap to the screen for studying a single deck.
     * @param {string} deckName The name of the deck to study.
     */
    async function initStudying(deckName) {

        const deckToStudy = appState.decks[deckName];
    
        // Check if cards in deck
        if(!deckToStudy || deckToStudy.cards.length === 0) {
            console.error(`Deck "${deckName}" has no cards to study`);
            alert("Error: No cards to study in deck");
            initDeckViewScreen(); // Go back to deck list if no cards to study
            return;
        }

        flashcardApp.replaceChildren();

        // Create to store and modify values 
        const studyState = {
            deck: deckToStudy,
            curCardIndex: 0,
            frontShowing: true,
            totalCards: deckToStudy.cards.length,

        }
        
        const deckStudyContainer = document.createElement("div");
        deckStudyContainer.id = "deck-study-screen";
        deckStudyContainer.innerHTML = `
            <header class="deck-study-header">
                <h2 id="deckStudyName"></h2>
                <div class="study-actions">
                    <button id="shuffleDeckBtn" class="button">Shuffle Deck</button>
                    <button id="endStudyBtn" class="button">End Study Session</button>
                </div>
            </header>

            <section class="study-main">

                <-- Could add some counter/view of which card out of total we are on -->
                <div class="study-progress">
                </div>

                <article id="studyCardContent" class="study-card">
                    <p id="cardText"></p>
                    <p id="cardTime">Time: <span id="cardTime"></span> sec</p>
                </article>
                
                <div class="study-controls">
                    <button id="flipCardBtn" class="button primary">Flip Card</button>
                    <button id="prevCardBtn" class="button">Previous Card</button>
                    <button id="nextCardBtn" class="button">Next Card</button>
                    <button id="pauseTimerBtn" class="button">Pause Timer</button>
                    <button id="resetTimerBtn" class="button">Reset Timer</button>
                </div>
        `

        flashcardApp.appendChild(deckStudyContainer);

        // Element referencing
        const deckStudyNameEl = deckStudyContainer.querySelector("#deckStudyName");
        const shuffleDeckBtn = deckStudyContainer.querySelector("#shuffleDeckBtn");
        const endStudyBtn = deckStudyContainer.querySelector("#endStudyBtn");

        const cardTextEl = deckStudyContainer.querySelector("#cardText");
        const cardTimeEl = deckStudyContainer.querySelector("#cardTime");

        const flipCardBtn = deckStudyContainer.querySelector("#flipCardBtn");
        const prevCardBtn = deckStudyContainer.querySelector("#prevCardBtn");
        const nextCardBtn = deckStudyContainer.querySelector("#nextCardBtn");

        // Not sure about allowing users to control timer functionality 
        /*
        const pauseTimerBtn = deckStudyContainer.querySelector("#pauseTimerBtn");
        const resetTimerBtn = deckStudyContainer.querySelector("#resetTimerBtn");
        */

        // CardText, CardTime, handled in displaying current card
        function displayCurrentCard() {
            const currentCard = studyState.deck.cards[studyState.curCardIndex];
            if(currentCard === null) {
                return;
            }
            
            // Check which side is meant to be displayed
            if(studyState.frontShowing) {
                cardTextEl.textContent = currentCard.frontText;
                flipCardBtn.textContent = "Show Back";

            } else {
                cardTextEl.textContent = currentCard.backText;
                flipCardBtn.textContent = "Show Front";
            }

            // Display time
            cardTimeEl.textContent = currentCard.time;

            prevCardBtn
        }

        deckStudyNameEl.textContent = deckToStudy.deckName;

        shuffleDeckBtn.addEventListener("click", () => {
            const shuffleCheck = shuffleCards(studyState.deck.cards);
            if(shuffleCheck)
            {
                studyState.curCardIndex = 0;
                studyState.frontShowing = true;
        ...
        endStudyBtn.addEventListener("click", () => {
            ...
            // TODO: Potentially modify confirmation 

            const confirmEnd = confirm("Are you sure you want to end this study session?");
    
            if (confirmEnd) {
                initDeckViewScreen(deckToStudy);
            }            
        });

        flipCardBtn.addEventListener("click", () => {
            studyState.frontShowing = !studyState.frontShowing;
            displayCurrentCard();

        })

        prevCardBtn.addEventListener("click", () => {
            if(studyState.curCardIndex > 0) {
                
                studyState.curCardIndex--;
                studyState.frontShowing = true;
        ...
        nextCardBtn.addEventListener("click", () => {
            if(studyState.curCardIndex < studyState.totalCards) {
